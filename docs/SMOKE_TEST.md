...
- **1×1 placeholder artwork file.** `Assets/PlaceholderArt.png` is a 1×1 transparent PNG (70 bytes, valid PNG magic, `(0, 0, 255, 127)` pixel). It IS bound as a `Binding.FallbackValue` at `Views/MainWindow.xaml:50-58`. Because it is 1×1 and ~fully transparent, on `Artwork == null` the placeholder contributes nothing visible and the artwork border background (`#26282E`, `ArtworkBackgroundBrush` from `App.xaml:26`) is what the user sees — i.e. an empty-state look, not a visible fallback. The earlier wording that called the wiring "not bound in XAML" was wrong about both the wiring and the border color (`#34373D` is `BadgeBorderBrush`, not `ArtworkBackgroundBrush`). If the empty-state look is too sparse, replace the PNG with a real opaque glyph (or stack a vector overlay above the `Image`) — bounded, isolated.
