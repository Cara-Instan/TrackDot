...
- **AllowsTransparency rendering tradeoff.** The popover uses `WindowStyle=None` + `AllowsTransparency=True` + a rounded `Border` (Task 7). On some GPUs / drivers this routes through WPF software rendering and can increase idle CPU. If users report CPU in idle, swap to `WindowChrome` rounded corners (deferred to a future pass — see plan §1.2 / Decision point 2 in HANDOFF.md).
